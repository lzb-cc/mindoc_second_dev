...
import (
	...
	"yy.com/mindoc/utils"
	"yy.com/mindoc/utils/cryptil"
	"yy.com/mindoc/utils/filetil"
	"yy.com/mindoc/utils/gopool"
	...
	if doc.Release != "" && cdnimg != "" {
		...
		if err != nil {
			...
			query.Find("img").Each(func(i int, contentSelection *goquery.Selection) {
				if src, ok := contentSelection.Attr("src"); ok && strings.HasPrefix(src, conf.WorkingDirectory+"/uploads/") {
